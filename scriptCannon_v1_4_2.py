...
def ScriptCannonUI():
    ...
    cmds.window('scriptCannonWindow', t='Script Cannon v1.4.1', ret=True, rtf=True, bgc=(1.0,0.4,0.0), width=600, height=710)
    #setup Tabs
    #setup scroll window
    cmds.scrollLayout('scrollSettings', width=600, height=710, vst=16)
    #set up main column
    ...
    cmds.formLayout(form, e=True, attachForm=((tabs, 'right', 0), (tabs, 'left', 0), (tabs, 'bottom', 0), (tabs, 'top', 0)))
    
    #setup two columns for respective attributes
    cannon = cmds.rowColumnLayout(numberOfColumns=1)
    ...
    '''
    #set up script graph
    #Currently, this does not work.  It has been disabled until such time as I can get the fuckin' thing to work!
    ...
    #close Tab 1
    cmds.setParent('..')
    repeater = cmds.rowColumnLayout('regularFunctions', numberOfColumns=1)
    cmds.rowColumnLayout(numberOfColumns=1)
    cmds.text('1. Batch Renamer')
    cmds.text('2. Hierarchy Builder')
    cmds.text('3. ...')
    ...
    cmds.tabLayout(tabs, e=True, tabLabel=((cannon, 'Cannon'), (repeater, 'Repeater')))
...
def setAttributes(type, objs):
    ...
    if attributeCollection == 'driver':
        ...
        collectAttributes = ca.findAttributes()
